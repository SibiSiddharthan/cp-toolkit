...
struct forward_prefix_sums_2d
{
		...
		requires(M != 0)
	{
		for (uint32_t i = 0; i < n; ++i)
		{
			...
			{
				this->prefix[i][j] =
					((M + (this->prefix[i][j] + this->_get(i - 1, j) + this->_get(i, j - 1))) - this->_get(i - 1, j - 1)) % M;
			}
		}
	}
		...
		requires(M != 0)
	{
		...
		if ((top - 1) < this->n && (left - 1) < this->m)
		{
			result = (result + this->prefix[top - 1][left - 1]) % M;
		}

		if ((left - 1) < this->m)
		{
			result = ((M + result) - this->prefix[bottom][left - 1]) % M;
		}

		if ((top - 1) < this->n)
		{
			result = ((M + result) - this->prefix[top - 1][right]) % M;
		}
...
struct backward_prefix_sums_2d
{
		...
		requires(M != 0)
	{
		for (uint32_t i = n; i != 0; --i)
		{
			...
			{
				this->prefix[i - 1][j - 1] =
					((M + (this->prefix[i - 1][j - 1] + this->_get(i, j - 1) + this->_get(i - 1, j))) - this->_get(i, j)) % M;
			}
		}
	}
		...
		requires(M != 0)
	{
		...
		if ((bottom + 1) < this->n && (right + 1) < this->m)
		{
			result = (result + this->prefix[bottom + 1][right + 1]) % M;
		}

		if ((right + 1) < this->m)
		{
			result = ((M + result) - this->prefix[top][right + 1]) % M;
		}

		if ((bottom + 1) < this->n)
		{
			result = ((M + result) - this->prefix[bottom + 1][left]) % M;
		}
